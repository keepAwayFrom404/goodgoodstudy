...
- new 的实例对象都有一个 constructor 属性，指向构造函数，最初用来标志对象类型，但是，检测对象类型还是instanceof更可靠一些
- 当作普通函数调用会挂载到全局对象上
...
### 07.29 递归
1. 递归
（1）在递归函数声明的时候，可使用arguments.callee访问一个正在执行的函数的指针，而不用担心如何调用函数而出现问题；
（2）在严格模式下，可使用命名函数表达式进行递归调用const funName = function f(){}
2. 闭包
