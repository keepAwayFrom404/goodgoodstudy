...
- new 的实例对象都有一个 constructor 属性，指向构造函数，最初用来标志对象类型，但是，检测对象类型还是 instanceof 更可靠一些
- 当作普通函数调用会挂载到全局对象上
...
## 第七章 函数表达式（没看太懂实际的作用在哪）

### 07.29 递归

1. 递归
   （1）在递归函数声明的时候，可使用 arguments.callee 访问一个正在执行的函数的指针，而不用担心如何调用函数而出现问题；
   （2）在严格模式下，可使用命名函数表达式进行递归调用 const funName = function f(){}
2. 闭包
   （1）闭包就是有权访问另一个函数作用域的变量的函数，比如在函数内部定义函数

### 07.30 this、内存泄漏

（2）当某个函数被调用会创建一个执行环境及相应的作用域链，然后使用 arguments 和其它参数来初始化函数的活动对象
（3）后台的每个执行环境中都有一个表示变量的对象--变量对象，全局环境的变量对象始终存在，而局部的变量对象只在调用期间存在
（4）作用域链只是一个指针的列表，只是引用不实际包含变量对象
（5）闭包就是函数内部内部的函数访问了外部的属性或者方法，导致外部函数的对象得到引用而不会立即释放，需要等到闭包函数释放才能释放，所以会占用更多的内存空间
（6）this 是在运行时基于函数的执行环境绑定的，匿名函数的执行环境具有全局性，所以 this 一般指向 window
（7）内部函数搜索 this 和 arguments 时，只会搜索到其活动对象为止，因此永远不会直接访问外部函数的这两个变量

3. 模仿块级作用域
   （1）函数声明后不能带圆括号，而函数表达式可以，这也是立即执行函数套括号的原因
   （2）使用立即执行函数不会向全局添加变量，且不存在指向匿名函数的引用，只要函数执行完毕就会立即销毁其作用域链
4. 私有变量
   （1）我们把有权访问私有变量或者私有方法的公有方法成为特权方法；
   （2）在构造函数中定义特权方法：缺点是必须使用构造函数模式达到这个目的
   （3）使用静态私有变量实现特权方法：属性会变得所有实例共享，
   （4）多查找一个作用域在一定程度上会影响查找速度，这是闭包和私有变量的不足之处
   （5）模块模式，为单例创建私有的变量和特权方法；单例指只有一个实例的对象，使用函数表达式创建立即执行函数，在函数当中添加私有的属性和方法，最后返回一个对象，该对象包含有可以访问私有属性和方法的特权方法
5. 小结
   （1）函数表达式无需对函数明名，从而实现动态编程；匿名函数也称拉姆达表达式
   （2）函数表达式的特点：可创建匿名函数；无法确定如何引用函数饿的情况下，递归会比较复杂；递归应该始终使用 arguments.callee 来递归调用自身，因为函数名可能会发生变化
   （3）函数内部定义函数就创建了闭包，闭包有权访问外部函数内部的变量，因为闭包的作用域链包含着自己的作用域，包含函数的作用域还有全局的作用域；函数返回一个闭包时，这个函数的作用域会一直保存到闭包不存在为止；模仿块级作用域：创建一个立即执行函数；

## 第八章 BOM

1. window 对象
   （1）window 对象是 js 访问浏览器的一个接口，同时是 es 规定的 Global 对象，这意味着网页中定义的任何一个对象、变量或者函数都以 window 作为其 Global 对象
   （2）如果页面包含框架，则每个框架都拥有自己的 window 属性，并保存在 frames 集合中，
   （3）不同 frames 里面使用的构造函数不同，会影响到对跨框架传递的对象使用 instanceof 操作符
   （4）screenLeft，screenTop 访问窗口位置
