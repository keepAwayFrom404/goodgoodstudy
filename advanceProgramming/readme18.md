...
1. 安全的类型检测
   （1）使用 Object 的 toString 方法，都会返回一个[object NativeConstructorName]格式的字符串，Object.prototype.toString.call()
   （2）由于原生数组的构造函数名和作用域无关，因此可以和 toString 返回一致的值
   （3）不能检测非原生构造函数的构造函数名
   （4）开发人员定义的任何构造函数返回[object Object]
2. 作用域安全的构造函数
   （1）存在问题的原因是对于构造函数没有使用 new 操作符调用而将一些属性和方法绑定到 window 导致覆盖了 window 的一些属性
   （2）作用域安全的构造函数在进行任何更改前，确定 this 对象是正确类型的实例，如果不是，创建新的实例并返回
3. 惰性载入函数
   （1）函数在调用时再处理函数，将处理函数使用函数表达式表示，最后返回函数表达式的执行对象
   （2）第二种是声明函数时就指定适当的函数，使用立即执行函数，将执行结果赋值给变量
   （3）普通判断函数在每次调用都会重新判断，惰性载入函数只在执行分支代码时牺牲一部分性能，分支判断仅发生一次
4. 函数绑定
   （1）bind 函数：接受一个函数和一个环境，返回一个在给定环境中调用给定函数的函数，并将所有参数原封不动的传递进去
   （2）被绑定函数与普通函数相比有更多的开销，需要更多内存，多重函数调用稍微慢一些，最好必要时使用
5. 函数柯里化  
   （1）用于创建一个已经设置好了一个活或多个参数的函数，基本方法和函数绑定一样：使用一个闭包返回函数，区别在于，当函数被调用时，返回的函数还需要设置一些传入的参数
   （2）柯里化函数的创建步骤：调用另一个函数并为它传入要柯里化的函数和必要参数
   （3）不应该滥用，每个函数都会带来额外的开销

## 22.2 防篡改对象

1. 不可拓展对象
   （1）使用 Object.preventExtensions 方法可以禁止给对象添加属性和方法，对于已经存在的对象依然可以修改或者删除
   （2）Object.istExtensible 判断对象是否可以拓展
2. 密封的对象
   （1）Object.seal 方法密封对象，密封对象不可拓展，而且已有成员的 Configurable 会被设置为 false，这就意味着不能删除属性和方法，因此不能使用 Object.defineProperty 方法将数据属性修改为访问器属性，或者相反；属性值是可以修改的
   （2） Object.isSealed 检测对象是否密封
   （3）Object.isExtensible 检测密封对象会返回 false
3. 冻结的对象
   （1）Object.freeze 最严格的防篡改级别，既不可扩展，又是密封的，Writable 会被设置为 false，如果定义 Set，访问器属性是可写的；
   （2）Object.isFrozen 检测是否冻结，
   （3）Object.isSealed 返回 true，Object.isExtensible 返回 false

## 22.3 高级定时器

（1）可以吧 js 代码想像成在时间线上执行的
（1）定时器指定的时间间隔表示什么时候将定时器的代码添加到队列中，而不是何时实际执行代码
（2）队列中的所有代码都要等到 js 进程空闲之后才能执行，而不管他们是如何添加到队列中的

1. 重复的定时器 setInterval
   （1）定时器代码规则的插入到代码中,存在的问题是，定时器代码再再次添加之前还没有完全执行，结果导致定时器运行好几次而之间没任何停顿（是指定时器代码执行的时间比较长），这个问题 js 引擎自动解决，当使用 setInterval 时，仅当没有该定时器的任何代码实例时，才将定时器添加到任务队列中
   （2）重复定时器的两个问题：某些间隔会被跳过；多个定时器的代码执行之间的间隔会比预期小
   （3）可以使用链式 setTimeout 解决上面的两个问题
...
（4）每个浏览器窗口、标签页或者 frame 都有自己的事件队列

2. Yielding Processes
   （1）脚本长时间运行的原因：过长过深嵌套的函数调用、进行大量处理的循环
   （2）如果该处理并不是必须同步执行的且不用按顺序执行，可以使用定时器分隔该循环，叫做数组分块技术。
   （3）数组分块的重要性在于它可以把多个项目处理在执行队列上分开，在每个处理完后，给浏览器处理其他事物的机会，可以避免长时间运行的脚本错误，一旦一个函数的运行大于 500ms 就要考虑分块
...
3. 函数节流
   （1）函数节流的基本思想：某些代码不可能在没有间断的情况下连续重复执行；第一次调用函数指定一个定时器，在一定时间之后执行，当第二次调用该函数，清除前一个并新建另一个，如果前一个已经执行过了后一个就没有意义了，然而前一个未执行，就会将其替换为新的定时器；
   （2）目的在于只有执行函数的请求停止了一段时间之后才执行
...
（1）自定义事件的背后逻辑是创建一个管理事件的对象，让其他对象监听那些事件

## 22.5 拖放

（1）创建一个绝对定位元素，使其可以用鼠标移动（鼠标拖尾）

## 22.6 小结

（1）js 的函数非常强大，因为他们是第一类对象，使用闭包和函数环境切换，可以有很多使用函数强大的方法，可以创建作用域安全的构造函数，确保缺少 new 调用也不会改变错误的环境对象
（2）惰性载入函数，将任何代码分支推迟到第一次调用函数的时候
（3）函数绑定创建始终在指定环境运行的函数；柯里化可以创建已经填了某些参数的函数
（4）将绑定和柯里化组合，能够创造任意环境中以任意参数执行任意函数的方法
（5）创建防篡改对象：

- 不可拓展对象：不可给对象添加新的属性和方法
- 密封对象：也是不可以拓展，不可删除已有的属性和方法
- 冻结对象：也是密封对象，不允许重写对象的成员
